...
int build_filters(Pipeline *p, const char *ass, const char *fonts) {
    ...
    char args[256];
    AVRational tb = p->decoder->time_base.num ? p->decoder->time_base : av_make_q(1, AV_TIME_BASE);
    ...
    std::string filt = "subtitles=filename=";
    filt += escape_filter_path(ass);
    filt += ":fontsdir=";
    filt += escape_filter_path(fonts);
    filt += ":charenc=UTF-8:original_size=";
    filt += std::to_string(p->decoder->width);
    filt += "x";
    filt += std::to_string(p->decoder->height);
    filt += ",format=yuv420p";
...
int process(Pipeline *p, long duration_ms) {
    ...
    while ((err = av_read_frame(p->in, p->ipkt)) >= 0) {
        ...
        if (p->ipkt->stream_index == p->video_in) {
            ...
            av_packet_rescale_ts(
                    p->ipkt,
                    p->in->streams[p->audio_in]->time_base,
                    p->out->streams[p->audio_out]->time_base);
            p->ipkt->stream_index = p->audio_out;
            err = av_interleaved_write_frame(p->out, p->ipkt);
            av_packet_unref(p->ipkt);
            if (err < 0) {
                return err;
            }
    ...
    av_buffersrc_add_frame_flags(p->src, nullptr, 0);
