...
void burn_forward_log(const char *line) {
    ...
    std::lock_guard<std::mutex> lock(g_cb_mutex);
    JNIEnv *env = env_for_current_thread();
    if (env == nullptr || g_engine == nullptr || g_on_log == nullptr) {
        return;
    }
...
void burn_forward_progress(long time_ms) {
    std::lock_guard<std::mutex> lock(g_cb_mutex);
    JNIEnv *env = env_for_current_thread();
    if (env == nullptr || g_engine == nullptr || g_on_progress == nullptr) {
        return;
    }
